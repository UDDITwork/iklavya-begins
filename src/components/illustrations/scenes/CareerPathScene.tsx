  ...
  return (
    <svg viewBox="0 0 400 300" className={className} fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      {milestones.map((m, i) => (
        ...
        >
          {/* Marker post */}
          <line x1={m.x} y1={m.y} x2={m.x} y2={m.y + 15} stroke="#1E40AF" strokeWidth={1.5} strokeOpacity={0.12} />
          {/* Marker circle */}
          <circle cx={m.x} cy={m.y} r={10} fill="url(#pathFill)" stroke="#1E40AF" strokeWidth={1.5} strokeOpacity={0.15} />
          {/* Inner dot */}
          <motion.circle
            cx={m.x} cy={m.y} r={3.5}
            fill="#1E40AF" fillOpacity={0.2}
